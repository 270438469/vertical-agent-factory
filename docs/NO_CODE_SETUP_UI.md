...
5. 期望的每分钟调用上限和每月请求额度。
6. 若启用金融真实数据，准备 Tushare、FRED 或 Alpha Vantage 密钥；若启用微信公众号，准备公网 HTTPS API 域名。
...
- “金融宏观分析”“A 股分析”“美股分析”分别对应三个只读金融研究任务；
- “发布金融简报”是高风险写操作，默认必须审批。

当前仓库已经安装 `research` 和 `finance` Domain Pack。金融任务不提供个性化荐股、目标价、收益承诺或自动交易；如果需要法务、客服等其他领域，应先安装对应 Domain Pack。
...
- “内置样例”无需密钥，适合首次部署、自测试和演示；
- “真实数据”按任务要求填写 Tushare、FRED、Alpha Vantage 密钥；
- 数据 API 与模型 API 相互独立，本地流程也会计算区间收益、历史波动率和最大回撤。
...
选择金融任务后可在此处启用微信公众号。页面会生成 Token 和回调 URL；把同一 Token 填入微信公众平台，当前版本选择明文模式。完整步骤见[金融分析 Agent 与微信公众号接入手册](FINANCE_AGENT_AND_WECHAT.md)。
